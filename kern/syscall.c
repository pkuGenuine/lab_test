...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	env->env_tf.tf_cs |= 3;	// run at code protection level 3
	env->env_tf.tf_eflags |= FL_IF;	// interrupt enabled
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if (!pg)	// fail to allocate new page
		return -E_NO_MEM;
	ret = page_insert(e->env_pgdir, pg, va, perm);
	if(ret)	// fail to allocate new page table
	{
		page_free(pg);	// free the page  allocated
		return ret;
	}
	return 0;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if (!pg)	// srcva is not mapped in srcenvid's address space
		return -E_INVAL;
	if ((perm | PTE_SYSCALL) != PTE_SYSCALL || (perm & (PTE_U | PTE_P)) != (PTE_U | PTE_P))
		return -E_INVAL;
	...
	int ret = page_insert(dste->env_pgdir, pg, dstva, perm);
	if (ret)
		return ret;
	return 0;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	// LAB 4: Your code here.
	struct Env *e;
	// set checkperm flag to 0 so any env is allowed to send IPC msgs to any other env
	int r = envid2env(envid, &e, 0);
	if (r) return r;
	if (!e->env_ipc_recving) return -E_IPC_NOT_RECV;
	if ((uint32_t)srcva < UTOP)
	{
		if (PGOFF((uint32_t)srcva)) return -E_INVAL;
		if ((perm | PTE_SYSCALL) != PTE_SYSCALL || (perm & (PTE_U | PTE_P)) != (PTE_U | PTE_P))
			return -E_INVAL;
		...
		if ((uint32_t)(e->env_ipc_dstva) >= UTOP)	// receiver does not ask for a page
			return 0;
		r = page_insert(e->env_pgdir, pg, e->env_ipc_dstva, perm);
		if (r) return r;
		e->env_ipc_perm = perm;
	}
	// the send succeeds, update the target's ipc field
	e->env_ipc_recving = 0;
